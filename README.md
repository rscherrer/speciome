...
**Important:** the data files do not contain information about time points or the number of individuals in each time point, for example. So, make sure you do not forget to request `time` and `population_size` in the order file, if you are saving data that need this information. The R functions we provide in `speciomer`, for example, expect to find `time` and `population_size` to create data tables with the right dimensions. Similar, the genetic architecture is often needed to make sense of the genetic data saved, so setting `archsave 1` may be a good idea for analysis down the line.
